...
工作流 0.3.0 的本地 MinerU 路线还需要兼容的 `mineru` 接入 Skill；0.3.1 源码已将作者的 Desk 接入 Skill 内置，不再要求另装。工作台 0.1.0 安装包仍固定使用 0.3.0，通过自身适配器调用 MinerU Desk，并使用主工作流中的导出脚本。不要把“工作台能够打开”“主 Skill 已安装”“PDF 可以解析”和“语言检查通过”当成同一件事。具体依赖、许可限制与检查方法见 [agent 安装指南](agent-install.md#4-语言依赖与-030-兼容处理)。
