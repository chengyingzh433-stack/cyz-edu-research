...
Operate as one workbench with explicit skill dependencies: `humanizer-zh` (syw2039/humanizer-zh) for Chinese academic language, `humanizer` for English, and the bundled [mineru Skill](references/mineru/SKILL.md) for the user's local MinerU Desk PDF parsing. For PDF conversion, read and follow that embedded Skill and its linked references; a separate global mineru installation is not required. For language Skills, resolve sibling skill directories first, then the available skill catalog; read their SKILL.md before use. Do not ask the user to route internal steps. Missing language dependencies must be disclosed, not silently replaced with qu-ai-wei; PDF parsing must prepare and use the user's Desk first. Missing Skill registration or a missing Desk installation is not permission to skip preparation. Only after documented recovery failure or an explicit user choice may native extraction run with --fallback-reason. Preserve the workbench's evidence, semantic-lock, and project-state contracts around these dependencies.
